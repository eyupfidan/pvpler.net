...
<title>Üye Ol | <?=$siteayarlari["siteadi"]?></title>
<meta name="rating" content="general"/>
<meta name="robots" content="all">
<meta name="robots" content="index,follow">
<meta name="revisit-after" content="1 days"/>
<meta name="description" content="Pvp serveler üye ol, PVPLER.NET Üyelik Sistemi."/>
...
<link rel="canonical" href="<?=$siteayarlari["siteadresi"]?>/kullanici/uyeol"/>
<meta name="x-canonical-url" content="/"/>

<meta property="og:locale" content="tr_TR" />
<meta property="og:type" content="website" />
<meta property="og:title" content="Üye Ol | <?=$siteayarlari["siteadi"]?>" />
<meta property="og:description" content="Pvp serveler üye ol, PVPLER.NET Üyelik Sistemi." />
<meta property="og:url" content="<?='http://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];?>">  
<meta property="og:site_name" content="<?=$siteayarlari["siteadi"]?>">  

<meta name="twitter:card" content="summary_large_image" />
<meta name="twitter:description" content="Pvp serveler üye ol, PVPLER.NET Üyelik Sistemi." />
<meta name="twitter:title" content="Üye Ol | <?=$siteayarlari["siteadi"]?>" />
...
if($_POST)
{
if($_POST["username"] == "" or $_POST["password"] == "" or $_POST["password2"] == "" or $_POST["email"] == "")
{
echo'
...
$email = $_POST["email"];
$varmibak=mysqli_fetch_object(mysqli_query($dbbaglanti,"select * from uye where kadi='$kadi' or email='$email'"));
if($varmibak){
echo'
<div class="alert alert-danger">

<div class="message">Bu üyelik bulunmaktadır.</div>
</div>
';
}
else
{
$ac =mysqli_query($dbbaglanti,"insert into uye (cerez,kadi,sif,email,rutbe,yonetici,durum) values('$cerez','$kadi','$sif','$email','acemi','hayir','aktif')") or die(mysqli_error());
if($ac)
{
$_SESSION["kullanici"] = $kadi;
echo'<div id="basarili"><center>Kayıt Gerçekleşti Yönlendiriliyorsunuz.</center></div>';
...
<div class="message">Şifreler uyuşmuyor.</div>
</div>
';
}
}
...
?>
			...
			<form class="form-fv" method="post" action="#" data-process="responseRedirect">
			    ...
			    minlength="3" maxlength="20" data-fv-stringlength-trim="true" required>
			  </div>
			  <div class="form-group">
			    <label for="email">Email</label>
			    <input type="text" class="form-control input-black" id="email" name="email" placeholder="Email adresiniz"
			    pattern="[^@]+@[^@]+\.[a-zA-Z]{2,6}" data-fv-stringlength-trim="true" required>
			  </div>
			  <div class="form-group">
			    <label for="password">Şifre</label>
			    <input type="password" class="form-control input-black" id="password" name="password" placeholder="Şifre"
			    minlength="6" maxlength="16" data-fv-stringlength-trim="true" required>
			  </div>
			  <div class="form-group">
			    <label for="password2">Şifre Tekrar</label>
			    <input type="password" class="form-control input-black" id="password2" name="password2" placeholder="Şifre tekrar"
			    data-fv-identical="true"  data-fv-identical-field="password" required>
			  </div>


			  <button type="submit" class="btn btn-default btn-orange">Üye Ol</button>
			</form>
		</div>
	</div>
